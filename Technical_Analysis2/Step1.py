    ...
    def exe_step1(self, con):
        ...
        for name in JongMok:
            ...
            if con == 'con1':
                # 조건1 : 유통주식수의 10% 이상
                for i in data.index:
                    if i >= 25: # 열번째까지 값은 제외하고 구하기
                        try:
                            if data.iloc[i]['거래량'] >= data.iloc[i]['상장주식수'] * 0.1 and data.iloc[i]['등락률'] >= 0 : # 상장주식수의 10% 이상의 거래량, 양봉
                                
                                for j in range(24):
                                    if data.iloc[i-j-1]['거래량'] >= data.iloc[i-j-1]['상장주식수'] * 0.05 : # 해당 봉의 25전 봉들은 상장주식수의 10% 미만의 거래량
                                        tf_num += 1
                ...
                # 조건 3: 거래대금이 시가총액의 5% 이상(유통주식을 상장주식수의 절반이라고 가정하면 5%로 계산)
                for i in data.index:
                    if i >= 40: # 40번째까지 값은 제외하고 구하기
                        try:
                            if data.iloc[i]['거래대금'] >= data.iloc[i]['시가총액'] * 0.05 :
                                
                                for j in range(39):
                                    if data.iloc[i-j-1]['거래량'] >= data.iloc[i-j-1]['시가총액'] * 0.05 : # 해당 봉의 40전 봉들은 시가총액의 5% 미만의 거래량
                                        tf_num += 1
        ...
        # 엑셀로 저장
        dataset.to_excel('F:/JusikData/API/Analy_Technical/Step1/'+con+'.xlsx', index=False)
